# Module: Richardson extrapolation
# Author: oanikienko
# Date: 31/12/2023
...
def richardson_extrapolation(fault_rates, noisy_expectation_values):
    ...
    for i in range(len(fault_rates)):
        ...
        mitigated_value += gamma_i*noisy_expectation_values[i]
...
if __name__ == "__main__":
    ...
    noiseless_expectation_value = richardson_extrapolation(fault_rates, noisy_expectation_values)

    plt.scatter(fault_rates, noisy_expectation_values, marker="x")

    plt.plot(0, noiseless_expectation_value, marker=".", label="richardson")
